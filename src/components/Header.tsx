...
function getLocaleUrl(targetLocale: Locale, testId?: string) {
  if (targetLocale === 'en') {
    return testId ? `/${testId}` : '/'
  }

  return testId ? `/${targetLocale}/${testId}` : `/${targetLocale}/`
}
        ...
        {selectedTest && (
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 min-w-0 overflow-hidden">
            ...
            <span className="text-xs text-neutral-500 hidden md:inline shrink-0">
              {getCategoryName(locale, selectedTest.category)}
            </span>
      ...
      {!isMobile ? (
        <div className="flex-1 flex justify-center min-w-0 mx-2">
          {timeRemaining !== undefined && timeRemaining !== null && (
            <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 text-sm font-mono transition-colors ${timeRemaining < 300 ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-neutral-900 text-neutral-300'}`}>
              <span className="text-neutral-500">
                {locale === 'es' ? 'Tiempo restante:' : 'Time remaining:'}
              </span>
              <span className={timeRemaining < 300 ? 'font-bold flex items-center gap-2' : ''}>
                {timeRemaining < 300 && <span className="animate-pulse w-2 h-2 rounded-full bg-red-500 hidden sm:block"></span>}
        ...
        timeRemaining !== undefined && timeRemaining !== null && (
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-white/10 text-xs font-mono ml-2 transition-colors ${timeRemaining < 300 ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-neutral-900 text-neutral-300'}`}>
            <span className={timeRemaining < 300 ? 'font-bold' : ''}>
              {Math.floor(timeRemaining / 60)}:{String(timeRemaining % 60).padStart(2, '0')}
            </span>
          </div>
        )
