...
// import AlgorithmShowcase from '@components/AlgorithmShowcase' // We don't need this for now since it's tests and not algos
...
export default function WelcomeScreen({ t, locale, onSelectTest }: WelcomeScreenProps) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 md:gap-6 min-h-0">
      ...
      <div className="max-w-md w-full bg-white/5 border border-white/10 rounded-xl p-6 text-center shadow-lg">
        <p className="text-sm text-neutral-400 mb-4">
          Welcome to the Candidate Testing Platform.
          Select a test from the sidebar on the left to start your evaluation.
        </p>
        <div className="flex justify-center flex-wrap gap-2 text-xs">
          <span className="px-3 py-1 bg-sky-500/10 text-sky-400 border border-sky-500/20 rounded-full">React</span>
          <span className="px-3 py-1 bg-red-500/10 text-red-500 border border-red-500/20 rounded-full">Angular</span>
          <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full">Junior</span>
          <span className="px-3 py-1 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-full">Middle</span>
          <span className="px-3 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded-full">Senior</span>
        </div>
      </div>
